...
touch /var/log/test/access_sdx_lc.log
touch /var/log/test/error_sdx_lc.log
touch /var/log/test/access_validator.log
touch /var/log/test/error_validator.log
touch /var/log/test/kytos.log
tail -f /var/log/test/kytos.log
pipenv install --dev
pipenv run gunicorn 'swagger_server.__main__:app' -w 4 -b 0.0.0.0:8080 \
        --daemon \
        --access-logfile var/log/test/access_sdx_lc.log \
        --error-logfile var/log/test/error_sdx_lc.log \
        --capture-output \
        --log-level='debug'

pipenv run gunicorn 'sdx_topology.validator.app:app' -w 4 -b 0.0.0.0:8000 \
        --daemon \
        --access-logfile var/log/test/access_validator.log \
        --error-logfile var/log/test/error_validator.log \
        --capture-output \
        --log-level='debug'
